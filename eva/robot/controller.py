...
class FrankaController:
    ...
    def update_command(self, command, action_space="cartesian_velocity", gripper_action_space="velocity", blocking=False):
        ...
        # === NEW: 是否用 velocity 驱动 ===
        if DEBUG:
            joint_key = "joint_velocity" if action_space.endswith("_velocity") else "joint_position"
            gripper_key = "gripper_velocity" if gripper_action_space == "velocity" else "gripper_position"
            self.update_joints(action_dict[joint_key], velocity=action_space.endswith("_velocity"), blocking=blocking)
            self.update_gripper(action_dict[gripper_key], velocity=(gripper_action_space == "velocity"), blocking=blocking)
        else:
            self.update_joints(action_dict["joint_position"], velocity=False, blocking=blocking)
            self.update_gripper(action_dict["gripper_position"], velocity=False, blocking=blocking)
    ...
    def create_action_dict(self, action, action_space, gripper_action_space="velocity", robot_state=None):
        ...
        if "joint" in action_space:
            ...
            if action_space == "joint_velocity":
                ...
                # —— NEW —— Forward Kinematics + Jacobian to obtain Cartesian info
                if DEBUG: 
                    pos, quat = self._robot.robot_model.forward_kinematics(torch.Tensor(action_dict["joint_position"]))
                    action_dict["cartesian_position"] = pos.tolist() + quat_to_euler(quat).tolist() # seems to be working
                    # New: copy from above
                ...
                # New: copy from above
                if DEBUG: 
                    pos, quat = self._robot.robot_model.forward_kinematics(torch.Tensor(action[:-1]))
                    action_dict["cartesian_position"] = pos.tolist() + quat_to_euler(quat).tolist() # seems to be working
                    cartesian_delta = pose_diff(action_dict["cartesian_position"], robot_state["cartesian_position"])
                    cartesian_velocity = self._ik_solver.cartesian_delta_to_velocity(cartesian_delta)
                    action_dict["cartesian_velocity"] = cartesian_velocity.tolist()
