...
class Episode(object):
    ...
    def __init__(self, cfg, init_obs):
        ...
        self.terminated = torch.empty((cfg.episode_length,), dtype=torch.bool, device=self.device)
    ...
    @classmethod
    def from_trajectory(cls, cfg, obses, actions, rewards, terminated=False, dones=None, masks=None):
        ...
        episode.terminated = torch.tensor(
            terminated, dtype=episode.terminated.dtype, device=episode.device
        )
    ...
    def add(self, obs, action, reward, terminated, done, mask=1.0, success=False):
        ...
        self.terminated[self._idx] = torch.tensor(terminated, dtype=torch.bool, device=self.device)
        self.dones[self._idx] = torch.tensor(done, dtype=torch.bool, device=self.device)
        self.masks[self._idx] = torch.tensor(mask, dtype=torch.float32, device=self.device)
...
class ReplayBuffer():
    ...
    def __init__(self, cfg, dataset=None):
        ...
        self._terminated = torch.zeros((self.capacity + cfg.horizon - 1,), dtype=torch.bool, device=self.device)
    ...
    def init_from_offline_dataset(self, dataset):
        ...
        copy_data(self._terminated, dataset['terminated'], n_transitions)
    ...
    def add(self, episode: Episode):
        ...
        self._terminated[idxs] = episode.terminated[:ep_len]
    ...
    def _get_obs(self, arr, idxs):
        ...
        if arr.ndim <= 2:  # if self.cfg.modality == 'state':
            return arr[idxs].to(self.device)
    ...
    def sample(self):
        ...
        terminated = self._terminated[idxs_in_horizon]
        done = self._done[idxs_in_horizon]

        if not action.is_cuda:
            action, reward, mask, terminated, done, idxs, weights = \
                action.to(self.device), reward.to(self.device), mask.to(self.device), terminated.to(self.device), done.to(self.device), idxs.to(self.device), weights.to(self.device)

        return obs, next_obs, action, reward.unsqueeze(2), mask.unsqueeze(2), terminated.unsqueeze(2), done.unsqueeze(2), idxs, weights
    ...
    def save(self, path):
        ...
        dataset = {
            ...
            "terminated": self._terminated[:sz].cpu().numpy(),
...
def get_realkoch_dataset_dict(cfg):
    assert cfg.task.startswith("gymnasium_realkoch")
    from lerobot.common.datasets.lerobot_dataset import LeRobotDataset, LeRobotDatasetMetadata
    import torch.utils.data as data
    import time
    from tqdm import trange
    repo_id="gym-lowcostrobot/koch_robot_dataset_scripted_v1.1"
    metadata = LeRobotDatasetMetadata(repo_id=repo_id, root=cfg.dataset_dir)
    # load obs, next obs, reward, done, terminated.
    delta_timestamps = {
        # "observation.rgb": [t / metadata.fps for t in range(2)],
        # "observation.depth": [t / metadata.fps for t in range(2)],
        "observation.arm_qpos": [t / metadata.fps for t in range(2)],
        "observation.estimated_target_pos": [t / metadata.fps for t in range(2)],
        # "observation.segmentation": [t / metadata.fps for t in range(2)],
        "observation.target_eepos": [t / metadata.fps for t in range(2)],
        "observation.gripper_blocked": [t / metadata.fps for t in range(2)],
        "observation.ee_pos": [t / metadata.fps for t in range(2)],
        "observation.log_is_success": [t / metadata.fps for t in range(2)],
        "action": [0],
        "reward": [0],
        "done": [0],
        "terminated": [0],
    }
    valid_keys = list(delta_timestamps.keys())
    
    print(f"Loading dataset from {cfg.dataset_dir}")
    dataset = LeRobotDataset(repo_id=repo_id, root=cfg.dataset_dir, delta_timestamps=delta_timestamps)
    print("Creating in-memory dataset cache...")
    # Create a custom Dataset class that holds all data in memory
    class InMemoryDataset(data.Dataset):
        def __init__(self, original_dataset, num_transitions):
            self.original_dataset = original_dataset
            self.data = []
            # Load all items into memory
            for i in trange(min(num_transitions, len(original_dataset)), desc="Loading dataset into memory"):
                item = original_dataset[i]
                episode_id = item['episode_index'].item()
                # if episode_id > 128:
                #     print(f"Skipping episode {episode_id} because it's new data.")
                #     break
                if cfg.filter_demos and original_dataset.meta.episodes_stats[episode_id]['reward']['max'] < 10:
                    print('todo: need to fix data where reward functions have changed over time.')
                    continue
                subset = {k: item[k] for k in valid_keys}
                self.data.append(subset)
            print(f"Loaded {len(self.data)} out of {len(original_dataset)} transitions.")
        def __len__(self):
    ...
    in_memory_dataset = InMemoryDataset(dataset, num_transitions=1e9)
    return in_memory_dataset

def get_dataset_dict(cfg, env, return_reward_normalizer=False, return_action_transform=False):
    ...
    if any([cfg.task.startswith(x) for x in ["xarm", "gymnasium_xarm", "gymnasium_koch"]]):
        dataset_path = os.path.join(cfg.dataset_dir, cfg.buffer_name)
        ...
        if any([cfg.task.startswith(x) for x in ["gymnasium_koch", "gymnasium_xarm"]]):
            dataset_dict['observations']['rgb'] = dataset_dict['observations']['rgb'].transpose(0, 3, 1, 2)
            dataset_dict['next_observations']['rgb'] = dataset_dict['next_observations']['rgb'].transpose(0, 3, 1, 2)
            dataset_dict['masks'] = np.ones_like(dataset_dict['dones'])
            if cfg.zero_state:
                dataset_dict['observations']['state'] = np.zeros_like(dataset_dict['observations']['state'])
            if cfg.zero_rgb:
                dataset_dict['observations']['rgb'] = np.zeros_like(dataset_dict['observations']['rgb'])
                dataset_dict['next_observations']['rgb'] = np.zeros_like(dataset_dict['next_observations']['rgb'])

            if cfg.modality == 'state':
                dataset_dict['observations'] = dataset_dict['observations']['state']
                dataset_dict['next_observations'] = dataset_dict['next_observations']['state']
            elif cfg.modality == 'pixels':
                dataset_dict['observations'] = dataset_dict['observations']['rgb']
                dataset_dict['next_observations'] = dataset_dict['next_observations']['rgb']

            print('\ndataset statistics:')
            for k, v in dataset_dict['metadata'].items():
                print(k, v)
            
            if cfg.task.startswith("gymnasium_koch") and return_action_transform:
                # normalize the actions
                action_min = np.min(dataset_dict['actions'], axis=0)
                action_max = np.max(dataset_dict['actions'], axis=0)
                # bound the actions to [-1, 1]
                dataset_dict['actions'] = 2 * (dataset_dict['actions'] - action_min) / (action_max - action_min) - 1
                dataset_dict['action_min'] = action_min
                dataset_dict['action_max'] = action_max
                # given a normalized action, we can recover the original action by
    ...
    if return_reward_normalizer:
        ret.append(reward_normalizer)
    if return_action_transform:
        ret.append(action_transform)
...
def get_reward_normalizer(cfg, dataset):
    ...
    """
    if cfg.task.startswith("xarm") or cfg.task.startswith("gymnasium_xarm"):
        return lambda x: x
