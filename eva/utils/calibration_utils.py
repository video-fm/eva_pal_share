...
def visualize_calibration(calibration_dict):
    ...
    assert len(calibration_dict) < (len(shapes) - 1), "Length mismatch in visualize calibration!"
    plt.clf()
...
class CharucoDetector:
    ...
    def calculate_target_to_cam(self, readings, train=True):
        ...
        for i in range(len(readings)):
            ...
            assert img_size == fixed_image_size, "Image size mismatch!"
    ...
    def augment_image(self, cam_id, image, visualize=False, visual_type=["markers", "axes"]):
        ...
        assert all([t in ["markers", "charuco", "axes"] for t in visual_type]), "in augment_image"
        if image.shape[2] == 4:
...
def calibrate_camera(
    ...
    if obs_pointer is not None:
        assert isinstance(obs_pointer, dict), "Provided obs_pointer is not dict"
    ...
    assert pause_time > (camera.latency / 1000), "pause_time too short"
    ...
    while True:
        ...
        # Get Action #
        action = controller.forward({"robot_state": state})[0]
